...
.controller("${widgetCode}Ctrl",function($scope,AppKit,$stateParams,$filter,$state,$rootScope,$ionicHistory){
	...
	$scope.loadIntentFollowupInfoData = function(){
		...
		$scope.followupInfo.custId = $stateParams.custId;
		$scope.followupInfo.orgName = $stateParams.custName;
		$scope.followupInfo.visitStateName = "初始化"
		$scope.followupInfo.visitState = 'init';
		if(undefined == $stateParams.orgId){
			$scope.followupInfo.orgId = "";
		}
	}
